...
public:
...
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr << " ctor: allocated ..."
              << std::endl;
#endif
  }
  ...
  SmartPtr(const SmartPtr<T> &ptr) {
    ...
    ++(*ref_count_ptr);
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr << " copy ctor: refed ..."
              << std::endl;
#endif
  }
   ...
   */
  SmartPtr(SmartPtr<T> &&ptr) noexcept
      : ref_count_ptr(std::move(ptr.ref_count_ptr)),
        data_ptr(std::move(ptr.data_ptr)) {
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr << " copy ctor: refed ..."
              << std::endl;
#endif
   ...
   */
  SmartPtr<T> &get_ref() { return *this; }
   ...
   */
  long get_ref_count() { return *ref_count_ptr; }

  /**
   * @brief set value
   * @param const T& val
   */
  void set_val(const T &val) { *data_ptr = val; }

  /**
   * @brief get value
   * @return reference to value
   */
  T &get_val() { return *data_ptr; }

  /**
   * @brief allocate and add reference count
   * @param ap another SmartPtr.clang-format.bak
   * @return reference to SmartPtr
   */
  SmartPtr<T> &alloc(SmartPtr<T> &ptr) {
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr << " ptr.dalloc ..."
              << std::endl;
#endif
    ptr.dalloc();
    ...
    ++(*ref_count_ptr);
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr
              << " allocated / ref incremented ..." << std::endl;
#endif
    return ptr;
  }
  ...
  void dalloc() {
#if TRACE
    std::cout << " @-> " << std::hex << data_ptr << " deallocate: ... "
              << std::endl;
#endif
    if (data_ptr != 0) {
      --(*ref_count_ptr);
#if TRACE
      std::cout << " @-> " << std::hex << data_ptr
                << " deallocate ref decremented ... " << std::endl;
#endif
      if (*ref_count_ptr == 0) {
